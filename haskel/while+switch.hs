...
instance Show Stm where
  ...
  show (Switch a cs) = "switch (" ++ show a ++ ")" ++ cases
    where cases = concat $ map showCase cs
...
sDS (Switch a cs)         = \state -> findCase (sAExp a state) cs state
  where findCase i ((i2, s):cs) state
          | i == i2 = sDS s state
          | otherwise = findCase i cs state
