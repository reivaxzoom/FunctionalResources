...
data AExp
  ...
  | Plus AExp AExp | Minus AExp AExp | Negate AExp | Conditional BExp AExp AExp
...
instance Show AExp where
  ...
  show (Conditional b x1 x2) = "("++ show b ++ " ? "++ show x1 ++ " : "++ show x2++ " ) "  
--  x := 0 < x ? x : 0 - x
...
sAExp (Conditional b x1 x2 ) s  
        |(sBExp b s)== True = (sAExp x1 s)
        |otherwise = sAExp x2 s 
...
test = "y" <=> (Conditional FF (lit 9) (lit 0))
test2 =sAExp (Conditional (TT) (var "x") (var "y")) aState
