...
%% {LPid, Bad, Good} =tw3:setup().
%% HPid=lists:nth(7, LPid).
%% HPid ! {["hola"],[], Bad,Good, 0, 0, self(),self()}.
%% lists:nth(7, LPid) ! {["hola"],[], Bad,Good, 0, 0, self(),[],self()}.
%% lists:nth(7, LPid) ! {["interesting", "His", "performance", "in", "the", "movie", "was", "amazing", "and", "funny"],LPid, Bad,Good, 0, 0, self(),self()}.

%%Normal Start
%%  {LPid, Bad, Good} =tw3:setup().
%%  tw3:start(rw:readTwits(),LPid, Bad, Good).
...
start([HTwit|TTwit],LPid=[HPid|TPid], Bad, Good) ->
	...
	HPid ! {HTwit,TPid, Bad,Good, 0, 0, self(),self()},
	io:format("Twit ~p\n",[HTwit]),
	timer:sleep(2000),
...
main(length(CheckList), [], CheckList,true).

main(0,[],0,_) -> io:format("finish Pid on main  list\n");
main(0,_,0,_) -> io:format("finish Twit on main  list\n");


main(0,LPid,_,First) when First==false ->
...
main(N, LPid, [HChkL|TChk],First) when is_integer(N),First==false, N > 0 ->
	 process_flag(trap_exit, true),
    Pid=spawn_link(?MODULE, validate,[HChkL,lists:last(LPid)]),
	M=lists:append(LPid, [Pid]),
    main(N - 1,M,TChk,false);

main(N, LPid, [HChkL|TChk],First) when is_integer(N), First==true, N>0->
	 process_flag(trap_exit, true),
    Pid=spawn_link(?MODULE, validate,[HChkL,self()]),
	M=lists:append(LPid, [Pid]),
    main(N - 1,M,TChk,false).
...
validate(Pattern, PrePid) ->
receive
		{_,[],_,_,X,Y,MainPid, _} -> 
				MainPid ! {X, Y,self()},
				validate(Pattern, PrePid);

		{Twit,[HPid|TPid],Bad,Good, X, Y,MainPid, PrevPid} -> 
				  ...
				  case lists:member(Pattern, Good) of
					...
					true ->
						   case lists:member(Pattern, Twit) of 
								...
								validate(Pattern, PrePid);
							   true ->
								  HPid ! {Twit,TPid,Bad,Good,X+1,Y,MainPid,self()},
