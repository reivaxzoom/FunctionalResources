...
read()->
	...
	ArList=proplists:get_all_values(B, Exports),
	io:format("The arity avaiable for it is ~p ~n",[ArList]),
	{ok, Arity} =  ask_arity(ArList),
	
	MT= [lists:flatten(io_lib:format("Provide ~w Parameter of ~w ~n", [X,Arity]))|| X <- lists:seq(1, Arity)],
	
	Params = read_par(Arity,MT), {A, B, Params}
    of 	{M, F, P} -> 
		try
			apply(M, F, P)
		catch
			error:badarg -> " bad argument";
			error:function_clause ->"function clause";
			error:badarith -> "Bad arity";
			error:{badmatch,V} -> io_lib:format("Evaluation of a match expression failed. The value ~p did not match.",[V])
		end
    catch
	...
	test(Arg1) -> "arg1".
	test(Arg1,Arg2) -> "arg2".
	test(Arg1,Arg2,Arg3) ->"arg3".
	test(Arg1,Arg2,Arg3,Arg4) ->"arg4".
	
	sum(Arg1,Arg2) -> Arg1+Arg2.
	sum(Arg1,Arg2,Arg3) ->Arg1+Arg2+Arg3.
	sum(Arg1,Arg2,Arg3,Arg4) ->Arg1+Arg2+Arg3+Arg4.


read_par(0,[]) ->
    [];
read_par(A,[H|T])->
	{ok, P} = io:read(H), %% element (2, ...)
    [ P | read_par(A-1,T)].

ask_arity(Arity) ->
	{ok, Ar}=io:read("Provide a arity --> "),
	case  lists:member(Ar, Arity)  of
		true -> {ok, Ar};
		false-> ask_arity(Arity)
	end.
...
%Logs
%% =========================================================
%% (BeadAssigments@Xavier-pc)2> seventh:read().
%% Provide a module name --> seventh.
%% Provide a function name --> test.
%% The arity avaiable for it is [1,2,3,4] 
%% Provide a arity --> 3.
%% Provide 1 Parameter of 3 
%% 3
%% .
%% Provide 2 Parameter of 3 
%% 2.
%% Provide 3 Parameter of 3 
%% 5.
%% "arg3"
%% (BeadAssigments@Xavier-pc)6> seventh:read().
%% Provide a module name --> erlang.
%% Provide a function name --> length.
%% The arity avaiable for it is [1] 
%% Provide a arity --> 1.
%% Provide 1 Parameter of 1 
%% [1,2,3,4,5,6].
%% 6
%% 
%% (BeadAssigments@Xavier-pc)7> seventh:read().
%% Provide a module name --> seventh.
%% Provide a function name --> sum.
%% The arity avaiable for it is [2,3,4] 
%% Provide a arity --> 3.
%% Provide 1 Parameter of 3 
%% 4.
%% Provide 2 Parameter of 3 
